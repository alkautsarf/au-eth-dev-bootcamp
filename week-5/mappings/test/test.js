...
describe("ContractMap", function () {
  let a1;
  let a2;
  beforeEach(async () => {
    const Contract = await ethers.getContractFactory("ContractMap");
    contract = await Contract.deploy();
    await contract.deployed();
  ...
  describe("after creating a user", () => {
    ...
    it("should not allow a transfer to a non-existent user", async () => {
      ...
      assert(
        ex,
        "Should not be able to transfer balance to non-existent user. Expected transaction to revert!"
      );
    });
    ...
    describe("after creating another user", () => {
      ...
      it("should be able to transfer to the new user", async () => {
        ...
        assert.equal(
          user1.balance,
          50,
          "Should subtract the amount from the sender's balance."
        );
        assert.equal(
          user2.balance,
          150,
          "Should add the amount to the recipient's balance."
        );
      });
      ...
      it("should not allow a larger transfer than in the users balance", async () => {
        ...
        assert(
          ex,
          "Cannot transfer more than in the user's balance. Expected transaction to revert!"
        );
      });
    });
  });
...
describe('ContractNested', function () {
  let contract;
  let s1;
  let s2;
  let a1;
  let a2;
  beforeEach(async () => {
      const Contract = await ethers.getContractFactory("ContractNested");
      contract = await Contract.deploy();
      await contract.deployed();
